...
const regs = {
	http: {
		topic: /^https?:\/\/girlsin\.technology\/topic\/\w*/,
		user: /^https?:\/\/girlsin\.technology\/user\/\w*/
	},
	gif: /.*\.gif$/
};
	...
	_onLinkPress(url) {
		...
		if (/^https?:\/\/.*/.test(url)) {
			if (regs.http.topic.test(url)) {
				let topicId = url.replace(/^https?:\/\/girlsin\.technology\/topic\//, '');
			...
			if (regs.http.user.test(url)) {
				let userName = url.replace(/^https?:\/\/girlsin\.technology\/user\//, '');
