	...
	render() {
		return (
			<View style={styles.container}>
				...
				/>
				<Text style={styles.title}>GIT
					<Text style={{fontSize:18,color:'rgba(255,255,255,0.6)'}}>
						{' v' + config.package.version}
					</Text>
				</Text>

				<TouchableOpacity onPress={() => utils.link(config.gitAbout)}>
					<Text style={styles.subTitle}>For girlsin.technology</Text>
				</TouchableOpacity>
				...
				<View style={styles.footer}>
					<TouchableOpacity onPress={() => this.props.router.toUser({
                        userName:config.author.gitName
                    })}>
						<Text style={styles.subTitle}>@Yi Sun</Text>
					</TouchableOpacity>
